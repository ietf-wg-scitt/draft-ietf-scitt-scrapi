...
Clients SHOULD treat 5xx HTTP status code responses as transient failures and MAY retry the same request without modification at a later date.
...
The Transparency Service MUST respond with a single COSE Key (as defined in {{Section 7 of RFC9052}}) serialized as `application/cbor` if a matching key is found.
If no matching key is found, the Transparency Service SHOULD respond with a 4xx-class status code (typically 404 Not Found).
